...
export interface Vacancy extends Tagged {
  ...
  /** Короткий сквозной номер вакансии (для ориентирования в откликах/уведомлениях админу) — присваивается вручную при публикации. */
  number?: number
}
...
export interface SavedVacancy {
  ...
  /** Как уведомлять об откликах на эту вакансию: сразу по каждому,
   *  ежедневной сводкой или не уведомлять вовсе. По умолчанию — сразу. */
  notifyMode?: 'instant' | 'daily' | 'off'
}
...
export interface EventItem extends Tagged {
  ...
  takeaways: string[]
  /** «Это мероприятие для вас, если» — 2-4 причины, кому подходит формат/тема. */
  audienceFit: string[]
  /** Дополнительные бонусы участникам (помимо того, что входит в тариф). */
  bonuses: string[]
